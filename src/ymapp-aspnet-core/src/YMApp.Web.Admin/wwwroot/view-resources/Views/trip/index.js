layui.use(['laydate', 'form', 'table'],
    function () {
        ...
        form.on('switch(state)', function (obj) {
            _tripService.changeAuditState({ id: this.value, state: obj.elem.checked ? 1 : 0 }).done(function () {
        ...
        table.on('tool(test)', function (obj) { //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
            ...
            if (layEvent === 'detail') {
                xadmin.open('编辑', '/trip/edit?id=' + data.id);
                ...
                xadmin.open('编辑', '/trip/edit?id=' + data.id, '', '', true);
            }
        });
